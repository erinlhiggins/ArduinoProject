...
MP3player.setPlaySpeed(0);
 ...
 int count;
  ...
  for(int i=0;i<count;i++){
    bData[0][i]=i+1;
    bData[1][0]=12;
    Serial.println(beatData[1][0]);
    bData[1][i]=beatCounter(i+1);
    Serial.println("poop");
  }
  sort(bData[1],bData[0],count);
  beatData[0]=bData[0];
  beatData[1]=bData[1];*/
...
if(buttonState3 == HIGH){
 ...
 if((current-start) >= 10000 && t==0 && testing==true && complete==false){
  t=bpmeq;
  bpm = bpmeq;
...
if(t!=0 && minh != 0 && looping == false){
  Serial.println("here to play song");
...
if(((current-start)/60000) > 1){
  bpm = bpmt/((current-start)/60000);
}

  if((current-timeloop) > 30000){
    Serial.println("checking if bpm is low");
       if(bpm < minh && t!=0 && minh != 0 && playCount < 5){
          playCount = playCount+1;
          MP3player.stopTrack();
          MP3player.playTrack(playCount);
        }
        if(bpm > maxh && t!=0 && maxh != 0 && playCount > 1){
          playCount = playCount-1;
          MP3player.stopTrack();
          MP3player.playTrack(playCount);
        }
